...
If you are targeting a runtime none of the above covers, the [Device WebSocket Protocol](/device-api/websocket-overview) and [WebSocket Channel Events](/device-api/websocket-events) references describe the connection, the join, and every message in both directions. The existing clients are also worth reading as reference implementations, particularly for reconnect behaviour and update progress reporting.
