---
title: "Build, Sign, and Upload Nerves Firmware to NervesHub"
sidebarTitle: "Firmware"
description: "How to build a fwup firmware file with Nerves, sign it with an Ed25519 key, and upload it to NervesHub for delivery to your device fleet."
---

NervesHub stores firmware as fwup-format `.fw` files — ZIP-based archives that carry a cryptographic Ed25519 signature alongside the filesystem image. Before a device applies an update, it verifies the signature against a trusted public key registered with NervesHub. This end-to-end signing chain ensures that no untrusted firmware can be installed on your devices, even if your update infrastructure is compromised.
...
If you have not yet created a signing key for your product, create one now. The `nh key create` command generates an Ed25519 keypair and registers the public key with NervesHub:
...
This writes the Ed25519 signature into the `.fw` archive in place. The signing key's private key material must be accessible to the host running this command.
