---
title: "nh CLI Command Reference: All Subcommands Documented"
...
Upload, inspect, download, and delete firmware artifacts. Firmware files use the fwup `.fw` format, signed with Ed25519 keys.
...
Ed25519 firmware signing keys are managed per-organization. A public key is stored in NervesHub; the private key never leaves your machine.

```bash
nh key list           # List all signing keys for the current org
nh key show <name>    # Show details for a signing key
nh key create <name>  # Generate and register a new Ed25519 signing key pair
...
```bash
nh migrate    # Import credentials and settings from the legacy Elixir mix CLI
```

Run `nh migrate` once after installing `nh` if you previously used `mix nerves_hub.*` tasks. The command reads your existing Mix CLI configuration and imports it into the `nh` config format without removing the original files.
