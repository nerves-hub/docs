---
title: "Sign and Upload NervesHub Firmware with Ed25519 Keys"
sidebarTitle: "Firmware Signing"
description: "How to create Ed25519 signing keys, embed public keys in your Nerves firmware, sign and upload firmware, and manage multiple keys for production safety."
---

Firmware signing ensures that only firmware produced by a party you trust can run on your devices. Before a device applies any OTA update, it verifies the Ed25519 signature embedded in the `.fw` file against a public key that was compiled into the firmware at build time. This verification happens entirely on the device — independently of NervesHub — so the chain of trust holds even if the server or the network is compromised.

## How It Works

NervesHub uses **Ed25519** keypairs via **libsodium**, implemented inside the `fwup` tool that packages Nerves firmware. Signing is a two-step distribution:

1. At build time, you embed the Ed25519 **public key** into your firmware image via the `fwup_public_keys` configuration. Every device that runs that firmware will only accept OTA updates signed by the corresponding private key.
...
Use the `nh` CLI to generate an Ed25519 keypair and register the public key with your NervesHub product in one step:
...
If you are using the legacy Mix-based workflow, you can create a key with:

```bash
mix nerves_hub.key create devkey --org my-org
```
