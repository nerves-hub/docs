---
title: "NervesHub PKI: Device Authentication and Firmware Trust"
sidebarTitle: "PKI"
description: "How NervesHub uses asymmetric cryptography to authenticate device connections with mTLS and to validate firmware integrity with Ed25519 signatures."
---
...
Firmware signatures use **Ed25519** via **libsodium**, implemented inside the `fwup` tool that Nerves uses to build and apply firmware archives. This is a separate mechanism from X.509: firmware signing keys are raw Ed25519 keypairs, not certificates, and they are not part of the TLS trust chain.
...
```text
[Build system]
  signs .fw with Ed25519 private key
        ↓
[NervesHub]
...
[Device]
  ...
  verifies Ed25519 signature locally using embedded public key
